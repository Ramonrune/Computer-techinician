...
<body>
	...
	<div id="corpo">
		<div>
			...
			<?php
			//aqui são mostradas todas as mensagens que foram excluidas
			include "../Conexao.inc";
			$resultado = mysqli_query ( $conexao, "SELECT * FROM lixeira" );
			if ($resultado === FALSE) {
				die ( mysql_error () );
			}
			@$numero = mysqli_num_rows ( $resultado );
			if ($numero == 0) {
				echo '<tr><td colspan="6" align="center">' . "Nenhuma Mensagem Excluida.</td></tr>";
			}
			
			while ( $row = mysqli_fetch_array ( $resultado ) ) {
				$id = $row ["id"];
				
				echo "<form action='mostralixeira.php?id=$id' method='GET'>";
				echo "<input type='hidden' name='id' value='$id'>";
				echo "<td>" . $row ["nome"] . "</td>";
				echo "<td>" . $row ["email"] . "</td>";
				echo "<td>" . $row ["assunto"] . "</td>";
				echo "<td><input type='submit' value='Ver' style='width:40px;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='excluilixeira.php?id=$row[id]'><img src='../Recursos/Imagens/delecao.png'></a></td>";
